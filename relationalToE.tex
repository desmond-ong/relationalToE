...
\title{ fixme: A relational lay theory of emotion: \\ Making and integrating multiple emotion attributions }
...
Talk about Moods vs Emotions

Talk about Lay theories

=> how are they integrated?

Talk about applying a formalism.


Analyze and maybe include the simultaneous experiment


\section{Introduction: Emotions and Moods}

	Emotions form a rich part of our mental lives -- we reason about not only our emotions but also of those around us, using our intuitive or lay theory of emotions (Ong, Zaki, \& Goodman; cites) \cite{Hareli2014}. In particular, we often make two kinds of emotional attributions to those around us: attributions that involve a specified cause (``Michael is feeling happy that he did well in his interview"), as well as more global, general attributions (``Henry is feeling sad"). These two classes of attributions made by naive observers coincide with a similar distinction made by affective scientists and philosophers between ``emotions" and ``moods" \cite{Sizer2000} (james' review, etc). However, there is no reason to think that the naive definitions map cleanly onto the scientific definitions. For instance, lay speakers do not often make a linguistic distinction between emotions and moods: statements like ``Henry is feeling sad" (with no specified cause) and ``Henry is in a melancholic mood" are interchangeable. Indeed, the lay person might not even know what ``affect" means, let alone that there different types of affective states! Yet, it is also important to study the naive theories that lay observers have, since it is those theories (and not scientific theories) that guide the layperson in their daily life. In the sections to follow, I briefly discuss how scientists parse these two distinct classes of affective states, and how we can use that as a starting point to study the corresponding lay conceptualizations of these states.

	Within affective science
	
	The layperson has an innate 
	


emotions and moods \cite{Sizer2000} 

propositional attitude theories (griffiths;  not explicitly by barrett, russell)
% are emotions propositional /attitudes/? that is, is it in the same category as "I believe that X"
lay theories, attributions using emotions 


central role of cognition in emotion (cite barrett, etc)
-- for emotions
...
\begin{Definition}
A Relational Emotion Attribution is a function $E$ that takes the agent $A$ and the event $X$, and returns real-valued numbers corresponding to the intensity of $A$'s emotions. $E(A,X)$ returns the emotions attributed to agent $A$ following event $X$.
\end{Definition}

\begin{Definition}
A Mood (or Overall Emotion) Attribution is a function $M$ that takes the agent $A$ and returns real-valued numbers corresponding to the intensity of $A$'s mood. $M(A)$ returns the current emotions attributed to agent $A$.
\end{Definition}
...
\subsection{Integrating to give mood attributions}
What is the relation between relational emotion and mood attribution?
...
\begin{figure}[htb!]
\includegraphics[width=\columnwidth]{images/Expt1SingleDouble.png}
\caption{ Experiment 1 Results. Single Double.}
\label{Expt1ResultsSingleDouble}
\end{figure}
...
The procedure is similar to Experiment 1, except that the character played one wheel at 10am (the \textit{temporally-earlier} wheel), and one wheel at 2pm (the \textit{temporally-later} wheel). The two wheel spins were presented as TV recordings (Fig. \ref{ParadigmFig3}). In the \textit{Congruent} condition, participants first saw the character spin the 10am wheel, then the 2pm wheel. In the \textit{Incongruent} condition, participants first saw the 2pm wheel, then the 10am wheel. In both conditions, similar to Experiment 1, they made attributions following each wheel, and then made an overall attribution after the \textit{temporally-later} wheel (``How does Bob feel overall immediately after the 2pm spin?").
